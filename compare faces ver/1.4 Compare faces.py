...
def compare_faces():
    ...
    siamese_model = tf.keras.models.load_model(r"siamesemodel_2.h5", custom_objects={"L1Dist":L1Dist, "BinaryCrossentropy":tf.losses.BinaryCrossentropy})
    ...
    if width == 1920 and height == 1080:
        width_win_scale = 1350
        height_win_scale = 1100
        
    elif width == 1366 and height == 768:
        width_win_scale = 800
        height_win_scale = 800


    # passport image preview
    passport_text = customtkinter.CTkButton(window, text="Passport Image", text_font = ("Roman", 20), fg_color="#000000", hover=False, border_width=3, corner_radius=0, height = 200)
    passport_text.grid(row=1, column=1, sticky=tk.NSEW)

    passport_label = customtkinter.CTkLabel(window)
    passport_label.grid(row=2, column=1, sticky=tk.NSEW, pady=10)

    passport_img = Image.open(r"C:\Users\16greenl\OneDrive - Tunbridge Wells Grammar School for Boys\Greenhalf Lukas\Coursework\Code\Siamese neural network\application_data\negative_images\41ef7ffe-5a98-11ed-8a6c-a3c80c43f39d.jpg")
    ...
    input_text = customtkinter.CTkButton(window, text="Compared Faces", text_font = ("Roman", 20), fg_color="#000000", hover=False, border_width=3, corner_radius=0, height = 200)
    input_text.grid(row=1, column=2, sticky=tk.NSEW)

    input_label = customtkinter.CTkLabel(window) 
    input_label.grid(row=2, column=2, sticky=tk.NSEW)
